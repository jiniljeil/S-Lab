...
KEYWORD * java_keyword[MAX];
// KEYWORD * java_sign[MAX]; 
int curr_key_word, n_keyword, */
int status;  
// int curr_sign, n_sign, status_sign; 
regex_t regex_expr; 
/*
KEYWORD * alloc_keyword(int k, char * regular_expr, char * color) { 
    java_keyword[k] = (KEYWORD*)malloc(sizeof(KEYWORD));
    java_keyword[k]->regular_expr = (char *)malloc(sizeof(char) * 200) ;
    java_keyword[k]->color =  (char*) malloc(sizeof(char) * 20);

    strcpy(java_keyword[n_keyword]->regular_expr, regular_expr) ;
    strcpy(java_keyword[n_keyword]->color, color) ;
    return java_keyword[k];
}

void Regular_Expression() {
    // java_sign[n_sign] = alloc_keyword(n_sign, "( .+)(\\s*)\\("); n_sign++;

    java_keyword[n_keyword] = alloc_keyword(n_keyword, "(public$|private$|protected$|default$|class$|interface$|extends$|implements$|static$|package$|import$)+", "green"); n_keyword++;
    java_keyword[n_keyword] = alloc_keyword(n_keyword, "(super$|this$|enum$|if$|else$|switch$|case$|break$|default$|catch$|throw$|new$|while$|do$|for$|true$|continue$|return$|finally$|synchronized$|assert$|final$)+", "green"); n_keyword++;
    java_keyword[n_keyword] = alloc_keyword(n_keyword, "(byte$|short$|int$|long$|float$|double$|boolean$|char$|void$)+", "red");
    // java_keyword[n_keyword] = alloc_keyword(n_keyword, "(")
        ...
        while(end < strlen(buf)) { 
            ...
            if( !strcmp(word, "import") || !strcmp(word,"package")) {
                ...
                if( !status ) {
                    ...
                    char * remainder_str = (char *)malloc(sizeof(char) * 100); 
                    int c = 0 ;
                    for(int i = idx; (buf[i] != '\n'); i++) {
